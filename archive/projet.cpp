...
bool brute(vector<int> &a, int empl, vector<int> &cnts, int cnt, int k) {
	if (n%2==0 && empl==n/2) empl++;
	if (n-empl<k || k<0) return 0;
	if (cnt==n) return 1;
	if (empl==n) return 0;
...
void solve() {
	...
	while (l<=r) {
		int mid=(l+r)/2;
		vector<int> a(beg); iota(all(a), 0); a.pb(n); if (n%2==0) a[1]=n/2;
		vector<int> cnts(n+1);
		for (int i=0; i<sz(a); i++) {
			for (int j=i+1; j<sz(a); j++) cnts[abs(a[i]-a[j])]++;
		}
		...
		cout << mid << endl;
		cout << flush;
		if (brute(a, beg, cnts, cnt, mid-1-beg)) {
			ans=mid;
			for (auto &u: a) u=n-u;
			sort(all(a));
			b.pb(a);
